...
def frame_to_tensor_rgb(frame_rgb, device=DEVICE, scale=SCALE):
    """
    Convert RGB frame from renderer to RAFT input tensor (1,3,H,W) on device.
    """
    rgb = frame_rgb  # renderer already outputs RGB
    if scale != 1.0:
...
def detect_hole_from_flow(flow_vis):
    """
    Use Canny edge detection on the flow visualization to find the 'hole'.
    Strategy:
      1. Convert flow_vis (RGB) to grayscale.
      2. Gaussian blur.
      3. Canny.
      4. Morphological close to fill gaps.
      5. Largest contour = hole candidate.
      6. Return overlay image and hole center (cx, cy) in flow_vis coordinates.
    """
    ...
    # Choose largest contour by area as hole
...
class FrameSaverThread(threading.Thread):
    """
    - Saves frames at a fixed rate (optional).
    - Runs RAFT optical flow between consecutive frames.
    - Runs Canny + contour to detect a hole.
    - Stores:
        * self.hole_center                : last detected center (in original image coords)
        * self.last_hole_overlay_big      : last overlay image (original size, RGB)
        * self.last_color_with_hole       : last original RGB frame where hole detected
    """
    def __init__(self, frame_dir, model_viz=None, device=None, hz=20.0, frame_count=100):
        ...
        self.hole_center = None  # in original image coordinates
        self.last_flow_vis = None

        # For saving the final detection explicitly
        self.last_hole_overlay_big = None   # RGB at original resolution
        self.last_color_with_hole = None    # original RGB frame that produced this detection
    ...
    def run(self):
        with torch.no_grad():
            ...
            while not self._stop_evt.is_set():
                ...
                if item is not None:
                    color_rgb = item  # renderer output, RGB
                    latest_panel = make_panel_rgb(color_rgb)

                    # ---- OPTICAL FLOW + HOLE DETECTION ----
                    if self.model_viz is not None:
                        ...
                        if self.prev_tensor is not None:
                            ...
                            flow_vis = flow_to_vis(flow)  # scaled resolution
                            ...
                            if center_small is not None:
                                ...
                                # Map from scaled coords back to original resolution
                                cx = int(cxs / SCALE)
                                cy = int(cys / SCALE)
                                self.hole_center = (cx, cy)

                                # Upsample overlay to original RGB size
                                ...
                                # Optional: write a debug file that gets overwritten
...
def main(renderer, model_path):
    ...
    try:
        ...
        # ---- Start moving LEFT/RIGHT while RAFT runs ----
        ...
        # pos_4 = currentPose['position'].copy()
        # targetPose4 = np.array([
        #     pos_4[0],
        #     pos_4[1] + 0.22,
        #     pos_4[2]
        # ])
        ...
        # # One left and right
        # time.sleep(0.5)
        # pos_5 = currentPose['position'].copy()
        # targetPose5 = np.array([
        #     pos_5[0],
        #     pos_5[1] - 0.22,
        #     pos_5[2]
        # ])
        ...
        # time.sleep(0.5)
        # pos_6 = currentPose['position'].copy()
        # targetPose6 = np.array([
        #     pos_6[0],
        #     pos_6[1] + 0.22,
        #     pos_6[2]
        # ])
        ...
        if saver_thread is not None:
            ...
            print(f"Scaled image coordinates: ({dy_world},{dz_world})")
            print(f"Target pose: ({Center_targetPose})")
